...
package network::f5::bigip::snmp::mode::listgtmpools;
...
sub manage_selection {
    ...
    foreach my $oid ($options{snmp}->oid_lex_sort(keys %{$snmp_result})) {
        next if ($oid !~ /^$mapping->{gtmPoolStatusAvailState}->{oid}\.(.*)$/);
        my $instance = $1;
        ...
        my $name = $self->decode_string_index(index => $instance);
    ...
    foreach my $name (sort keys %{$results}) {
        $self->{output}->output_add(
            long_msg => sprintf(
                ...
                $name, $results->{$name}->{status}, $results->{$name}->{state}
            )
        );
    }

    $self->{output}->output_add(severity => 'OK', short_msg => 'List GTM pools:');
...
sub disco_show {
    ...
    foreach my $name (sort keys %{$results}) {
        $self->{output}->add_disco_entry(
            name => $name, status => $results->{$name}->{status}, state => $results->{$name}->{state}
        );
    }
}
