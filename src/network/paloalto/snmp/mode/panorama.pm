...
sub set_counters {
    ...
    $self->{maps_counters}->{global} = [
        { label => 'admin-sessions', nlabel => 'management.admin.sessions.count', set => {
                key_values => [ { name => 'admin_sessions' } ],
                output_template => 'admin sessions: %s',
                perfdatas => [
                    { template => '%s', min => 0 }
                ]
            }
...
sub manage_selection {
    ...
    my $oid_panMgmtNumAdminSessions = '.1.3.6.1.4.1.25461.2.1.2.4.3.0';
    my $snmp_result = $options{snmp}->get_leef(
        oids => [$oid_panMgmtPanoramaConnected, $oid_panMgmtPanorama2Connected, $oid_panMgmtNumAdminSessions],
        nothing_quit => 1
    );

    $self->{global} = {
        admin_sessions => $snmp_result->{$oid_panMgmtNumAdminSessions}
    };
...
Check panorama connection status and admin sessions.

=over 8

=item B<--warning-admin-sessions>

Threshold warning for the number of active admin sessions.

=item B<--critical-admin-sessions>

Threshold critical for the number of active admin sessions.
