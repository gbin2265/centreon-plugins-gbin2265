...
package storage::scality::ring::mode::ringhealth;
...
sub custom_status_output {
    my ($self, %options) = @_;
    return sprintf("status is '%s' [state: %s]",
...
sub set_counters {
    ...
    $self->{maps_counters_type} = [
        { name => 'rings', type => 1, cb_prefix_output => 'prefix_ring_output',
          message_multiple => 'All rings are healthy', skipped_code => { -10 => 1 } },
    ];
    $self->{maps_counters}->{rings} = [
        # --- Status ---
        { label => 'status', type => 2,
          critical_default => '%{status} !~ /^ok$/i || %{state} !~ /^RUN$/i',
          set => {
        ...
        { label => 'stored', nlabel => 'ring.storage.stored.bytes', display_ok => 0, set => {
            key_values => [ { name => 'stored' }, { name => 'name' } ],
            output_template => 'stored: %s B',
            perfdatas => [ { template => '%d', unit => 'B', min => 0,
                label_extra_instance => 1, instance_use => 'name', cast_int => 1 } ],
          }
        },
        # --- Net (deduplicated) disk space ---
        { label => 'net-used', nlabel => 'ring.storage.net.used.bytes', display_ok => 0, set => {
        ...
        { label => 'net-available', nlabel => 'ring.storage.net.available.bytes', display_ok => 0, set => {
        ...
        { label => 'objects-unique', nlabel => 'ring.objects.unique.count', set => {
            key_values => [ { name => 'unique_objects' }, { name => 'name' } ],
        ...
        { label => 'objects-total', nlabel => 'ring.objects.total.count', display_ok => 0, set => {
            key_values => [ { name => 'total_objects' }, { name => 'name' } ],
        ...
        { label => 'object-avg-size', nlabel => 'ring.object.average.size.bytes', display_ok => 0, set => {
            key_values => [ { name => 'object_avg_unique_size' }, { name => 'name' } ],
        ...
        # --- Capacity planning ---
        { label => 'days-before-full', nlabel => 'ring.planning.days.before.threshold.count', set => {
            key_values => [ { name => 'days_before_threshold' }, { name => 'name' } ],
            output_template => '%d day(s) before capacity threshold',
            perfdatas => [ { template => '%d', min => 0,
                label_extra_instance => 1, instance_use => 'name' } ],
          }
        },
        { label => 'daily-growth', nlabel => 'ring.planning.daily.growth.bytes', display_ok => 0, set => {
            key_values => [ { name => 'daily_growth' }, { name => 'name' } ],
            output_template => 'daily capacity growth: %s B',
            perfdatas => [ { template => '%d', unit => 'B', min => 0,
                label_extra_instance => 1, instance_use => 'name', cast_int => 1 } ],
          }
        },
        { label => 'usage-growth', nlabel => 'ring.planning.usage.growth.bytes', display_ok => 0, set => {
            key_values => [ { name => 'usage_growth' }, { name => 'name' } ],
...
sub manage_selection {
    ...
    foreach my $ring (@{$items}) {
        ...
        $self->{rings}->{$name} = {
            ...
            free                      => $free,
            prct_used                 => $prct,
            stored                    => ($ring->{diskspace_stored}             // 0) || 0,
            # net diskspace
            diskspace_net             => ($ring->{diskspace_net}                // 0) || 0,
            diskspace_est_net_available => ($ring->{diskspace_est_net_available} // 0) || 0,
            # objects
            unique_objects            => ($ring->{unique_number_of_objects}     // 0) || 0,
            total_objects             => ($ring->{number_of_objects}            // 0) || 0,
            object_avg_unique_size    => ($ring->{object_average_unique_size}   // 0) || 0,
            # capacity planning
            days_before_threshold     => ($ring->{planning_days_before_threshold} // 0) || 0,
            daily_growth              => ($ring->{planning_daily_capacity_growth} // 0) || 0,
            usage_growth              => ($ring->{planning_usage_growth}          // 0) || 0,
            planning_threshold        => ($ring->{planning_threshold}             // 0.8) || 0.8,
        };
    }
...
Check Scality RING ring health and storage metrics.
...
(default: '%{status} !~ /^ok$/i || %{state} !~ /^RUN$/i').
You can use the following variables: %{status}, %{state}, %{name}

=item B<--warning-*> B<--critical-*>

Thresholds. Can be: 'nodes', 'usage', 'usage-prct', 'stored',
'net-used', 'net-available', 'objects-unique', 'objects-total',
'object-avg-size', 'days-before-full', 'daily-growth', 'usage-growth'.
